...
- **Note:** No changes to `tripsStore.ts` are needed. The store's `addEvent` and `updateEvent` accept `Omit<TripEvent, 'id' | 'destinationId' | 'createdAt'>` — once `lat` and `lng` are added to `TripEvent`, they are automatically included in this type.
...
3. For each consecutive pair `(a, b)` in `positioned`:
   ...
   - A gap warning `g` applies to this segment if the event referenced by `g.afterEventId` has a datetime in the range `[aDateTime, bDateTime)` — i.e., `eventById.get(g.afterEventId)` exists and its `date + ' ' + time >= aDateTime` AND `< bDateTime`. **Note:** `g.afterEventId` always points to the accommodation event immediately before the gap. When that accommodation event IS event `a` (i.e., it has coordinates and is the segment's start), its datetime equals `aDateTime` exactly — so the inclusive `>= aDateTime` boundary is intentional and correct. Do not change this to `>`.
...
- Renders a `<div ref={mapRef} className="flex-1 min-h-0">` that fills all remaining vertical space. Google Maps requires an explicit height on its container; `flex-1 min-h-0` achieves this when the parent is a flex column (see `TripDetailPage` changes below).
...
- Change the root div from `min-h-screen bg-background pb-20` to `h-screen bg-background flex flex-col`. Using `h-screen` + `flex flex-col` allows the map container to fill the remaining height via `flex-1`. The bottom nav is `fixed bottom-0` so `pb-20` is not needed on the root; the timeline's existing scroll behavior is unchanged.
...
- The header block (`pt-12 pb-4` title row + ViewToggle row) does not flex-grow — it stays its natural height.
- Render the body area as a `flex-1 overflow-auto` div:
  - `view === 'timeline'` → `<div className="flex-1 overflow-auto px-4">` containing the existing timeline JSX (unchanged)
  - `view === 'map'` → `<MapView events={sortedEvents} gaps={gaps} onEdit={openEditSheet} />` (MapView itself is `flex-1 min-h-0`)
...
Mock both the `@googlemaps/js-api-loader` module and the `google.maps` global. The loader mock makes `importLibrary` resolve immediately so the component initializes synchronously in tests:
...
Then stub the `google.maps` global:
